...
function logIn (logInInfo, callback) {
    var sql_login_check = 'select * from admin where member_id = ? and member_pwd = ?';
    dbPool.getConnection ( function (error, dbConn) {
        ...
        var logInMessage = {};
        dbConn.query(sql_login_check, [logInInfo.member_email, logInInfo.member_pwd], function (error, rows) {
            if (error) {
                dbConn.release();
                return callback(error);
            }
